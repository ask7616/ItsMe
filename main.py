...
class VideoTracker(object):
    def __init__(self, args):
        ...
        # 객체의 이전 프레임 위치를 저장할 변수 추가
        self.prev_locations = dict()
        ...
        # 카운트할 선의 좌표를 설정합니다. 여기서는 화면의 중앙을 기준으로 설정하였습니다.
        self.line = [(self.im_width // 2, 0), (self.im_width // 2, self.im_height)]
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                for box, identity in zip(bbox_xyxy, identities):
                    ...
                    # 이전 프레임 위치 확인
                    prev_center_x = self.prev_locations.get(identity, None)

                    # 이전 프레임에서 선을 넘지 않았으나 현재 프레임에서 선을 넘은 경우에만 카운트를 증가시킵니다.
                    if prev_center_x is not None and prev_center_x >= self.line[0][0] and center_x < self.line[0][0]:
                        self.count += 1
                        print(f"Count: {self.count}")

                    # 현재 프레임 위치를 이전 프레임 위치로 저장
                    self.prev_locations[identity] = center_x

                # 카운팅 선을 화면에 그립니다.
                cv2.line(img0, self.line[0], self.line[1], (0, 255, 0), 2)
